...
public final class MoreFiles {
     ...
     */
    public static ImmutableList<Path> listFiles(Path dir2) throws IOException {
    ...
    private static Iterable<Path> fileTreeChildren(Path dir2) {
        if (Files.isDirectory(dir2, LinkOption.NOFOLLOW_LINKS)) {
            try {
                return MoreFiles.listFiles(dir2);
            } catch (IOException e) {
    ...
    private static boolean isDirectory(SecureDirectoryStream<Path> dir2, Path name, LinkOption ... options) throws IOException {
        return dir2.getFileAttributeView(name, BasicFileAttributeView.class, options).readAttributes().isDirectory();
    }
    ...
    private static @Nullable Collection<IOException> deleteRecursivelySecure(SecureDirectoryStream<Path> dir2, Path path) {
        Collection<IOException> exceptions = null;
        try {
            if (MoreFiles.isDirectory(dir2, path, LinkOption.NOFOLLOW_LINKS)) {
                try (SecureDirectoryStream<Path> childDir = dir2.newDirectoryStream(path, LinkOption.NOFOLLOW_LINKS);){
                    exceptions = MoreFiles.deleteDirectoryContentsSecure(childDir);
                }
                if (exceptions == null) {
                    dir2.deleteDirectory(path);
                }
            } else {
                dir2.deleteFile(path);
            }
            return exceptions;
        } catch (IOException e) {
    ...
    private static @Nullable Collection<IOException> deleteDirectoryContentsSecure(SecureDirectoryStream<Path> dir2) {
        Collection<IOException> exceptions = null;
        try {
            for (Path path : dir2) {
                exceptions = MoreFiles.concat(exceptions, MoreFiles.deleteRecursivelySecure(dir2, path.getFileName()));
            }
            return exceptions;
        } catch (DirectoryIteratorException e) {
    ...
    private static @Nullable Collection<IOException> deleteDirectoryContentsInsecure(DirectoryStream<Path> dir2) {
        Collection<IOException> exceptions = null;
        try {
            for (Path entry : dir2) {
