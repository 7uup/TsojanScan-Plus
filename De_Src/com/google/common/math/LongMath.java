...
public final class LongMath {
    ...
    @GwtIncompatible
    public static int mod(long x, int m3) {
        return (int)LongMath.mod(x, (long)m3);
    }

    @GwtIncompatible
    public static long mod(long x, long m3) {
        if (m3 <= 0L) {
            throw new ArithmeticException("Modulus must be positive");
        }
        long result = x % m3;
        return result >= 0L ? result : result + m3;
    }
    ...
    private static enum MillerRabinTester {
        SMALL{
            ...
            @Override
            long mulMod(long a, long b, long m3) {
                return a * b % m3;
            }

            @Override
            long squareMod(long a, long m3) {
                return a * a % m3;
            }
        }
        ,
        LARGE{
            ...
            private long plusMod(long a, long b, long m3) {
                return a >= m3 - b ? a + b - m3 : a + b;
            }

            private long times2ToThe32Mod(long a, long m3) {
                int shift;
                int remainingPowersOf2 = 32;
                do {
                    shift = Math.min(remainingPowersOf2, Long.numberOfLeadingZeros(a));
                    a = UnsignedLongs.remainder(a << shift, m3);
            ...
            long mulMod(long a, long b, long m3) {
                ...
                long result = this.times2ToThe32Mod(aHi * bHi, m3);
                if ((result += aHi * bLo) < 0L) {
                    result = UnsignedLongs.remainder(result, m3);
                }
                result += aLo * bHi;
                result = this.times2ToThe32Mod(result, m3);
                return this.plusMod(result, UnsignedLongs.remainder(aLo * bLo, m3), m3);
            }
            ...
            long squareMod(long a, long m3) {
                long aHi = a >>> 32;
                long aLo = a & 0xFFFFFFFFL;
                long result = this.times2ToThe32Mod(aHi * aHi, m3);
                long hiLo = aHi * aLo * 2L;
                if (hiLo < 0L) {
                    hiLo = UnsignedLongs.remainder(hiLo, m3);
                }
                result += hiLo;
                result = this.times2ToThe32Mod(result, m3);
                return this.plusMod(result, UnsignedLongs.remainder(aLo * aLo, m3), m3);
            }
        };
        ...
        private long powMod(long a, long p, long m3) {
            long res = 1L;
            while (p != 0L) {
                if ((p & 1L) != 0L) {
                    res = this.mulMod(res, a, m3);
                }
                a = this.squareMod(a, m3);
                p >>= 1;
            }
            return res;
        }
