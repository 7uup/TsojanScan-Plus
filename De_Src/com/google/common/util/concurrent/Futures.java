...
extends GwtFuturesCatchingSpecialization {
    ...
    @Beta
    public static ListenableFuture<Void> submit(Runnable runnable2, Executor executor) {
        TrustedListenableFutureTask<Object> task = TrustedListenableFutureTask.create(runnable2, null);
        executor.execute(task);
        return task;
    }
    ...
    @CanIgnoreReturnValue
    @GwtIncompatible
    public static <V, X extends Exception> V getChecked(Future<V> future, Class<X> exceptionClass, Duration timeout2) throws X {
        return Futures.getChecked(future, exceptionClass, Internal.toNanosSaturated(timeout2), TimeUnit.NANOSECONDS);
    }
