...
public class AnnotationUtils {
    ...
    public static boolean equals(Annotation a1, Annotation a2) {
        ...
        try {
            for (Method m3 : type.getDeclaredMethods()) {
                if (m3.getParameterTypes().length != 0 || !AnnotationUtils.isValidAnnotationMemberType(m3.getReturnType())) continue;
                Object v1 = m3.invoke(a1, new Object[0]);
                Object v2 = m3.invoke(a2, new Object[0]);
                if (AnnotationUtils.memberEquals(m3.getReturnType(), v1, v2)) continue;
                return false;
            }
    ...
    public static int hashCode(Annotation a) {
        ...
        for (Method m3 : type.getDeclaredMethods()) {
            try {
                Object value = m3.invoke(a, new Object[0]);
                if (value == null) {
                    throw new IllegalStateException(String.format("Annotation method %s returned null", m3));
                }
                result += AnnotationUtils.hashMember(m3.getName(), value);
    ...
    public static String toString(Annotation a) {
        ...
        for (Method m3 : a.annotationType().getDeclaredMethods()) {
            if (m3.getParameterTypes().length > 0) continue;
            try {
                builder.append(m3.getName(), m3.invoke(a, new Object[0]));
            } catch (RuntimeException ex) {
                throw ex;
            } catch (Exception ex) {
                throw new RuntimeException(ex);
